...
namespace Elements
{
    public class ElementAbility : MonoBehaviour
    {
        ...
        private void Effect(IReadOnlyList<Element> elements)
        {
            ...
            {
                if (elements[0].ElementType == ElementType.WaterElement)
                    StartCoroutine(PeriodicEffect(elements.Count + elements.Count, _player));
                else if (elements[0].ElementType == ElementType.FireElement)
                    StartCoroutine(PeriodicEffect((elements[0].Damage + elements.Count) * _enemy.Resistance.GetPercentValue(elements[0]), _enemy));
                else if (elements[0].ElementType == ElementType.EarthElement)
                    StartCoroutine(EarthEffect(elements.Count / Divider));
                else if (elements[0].ElementType == ElementType.WindElement)
        ...
        private IEnumerator PeriodicEffect<T>(float value, T gameObject)
